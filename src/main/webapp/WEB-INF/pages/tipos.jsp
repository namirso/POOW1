        ...
        <div class="collapse navbar-collapse" id="navbarNavDropdown">
            <ul class="navbar-nav ms-auto">
                ...
                <c:if test="${sessionScope.usuarioLogado.permissao.nome eq 'ADMINISTRADOR'}">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="${pageContext.request.contextPath}/tipos">Tipos</a>
                    </li>
                    <li class="nav-item">
...
<main class="card container mt-4">
    <div class="p-3">
        ...
        <form action="tipos" method="post" class="mb-4 p-3">
            ...
            <input type="submit" class="btn btn-success" value="${tipo.id != 0 ? 'Alterar' : 'Cadastrar'}">
        </form>
        ...
        <div class="p-3">
            <table class="table table-striped table-dark table-bordered align-middle">
                ...
                <tbody>
                <c:forEach var="tipoF" items="${tipos}">
                    <tr>
                        <td>${tipoF.nome}</td>
                            ...
                            <c:choose>
                                <c:when test="${tipo.id != 0}">
                                    <button class="btn btn-primary btn-sm me-2" disabled
                                            ...
                                            title="Finalize a edição antes de editar outro tipo">Editar</button>
                                    <button class="btn btn-danger btn-sm" disabled
                                            data-bs-toggle="tooltip" data-bs-placement="top"
                                            title="Finalize a edição antes de excluir">Excluir</button>
                                </c:when>
                                <c:otherwise>
                                    <a href="${pageContext.request.contextPath}/tipos/editar/${tipoF.id}"
                                       class="btn btn-primary btn-sm me-2">Editar</a>
                                    <a href="${pageContext.request.contextPath}/tipos/excluir/${tipoF.id}"
